...
SuperMap.Handler.SmcPathMeasure = SuperMap.Class(SuperMap.Handler.Path, {
	...
	measureDistance : function(geometry) {
		var subLength = geometry.getGeodesicLength(init.map.getProjectionObject());
      ...
      if(!this.count) {
				...
				if(this.movePopup) {
					contentHtml = '<div class="smControlMeasureContent">'
						...
						+ '</div>'
						+ '<div class="MeasureEndDescript">안 끝마칩니다</div>'
						+ '</div>';
	    ...
	    // 	popup.attributes = {
			...
			if(evt.button == "2" && measureFunc.isMeasurePolygonOn === true) {
				this.rightclick(evt);
		        return true;
			}
      return false;
...
SuperMap.Handler.SmcPolygonMeasure = SuperMap.Class(SuperMap.Handler.Polygon, {
	...
	measureArea : function() {
		var geometry = this.geometryClone();
		var subLength = geometry.getGeodesicArea(this.map.getProjectionObject());
		var tempLength = subLength;
    	var unit = "";
			...
			if(!this.count) {
				...
				if(this.movePopup) {
					contentHtml = '<div class="smControlMeasureContent">'
						...
						+ '<span class="measureResUnit"></span>'
						+ '</div>'
						+ '<div class="MeasureEndDescript">마우스 오른쪽 버튼을 클릭하시면 끝마칩니다</div>'
						+ '</div>';
...
SuperMap.SmcPopup = SuperMap.Class(SuperMap.Popup, {
	...
	//팝업을 거리를 두고 그림
	offsetPixel : null,
	//panning 후 마우스 이동에 따른 offset 적용
	moved : false,
	/**
    ...
    moveTo: function(px) {
        if ((px != null) && (this.div != null)) {
	    	...
	    	if(this.moved){
		    	var left = this.map.layerContainerDiv.offsetLeft;
				var top = this.map.layerContainerDiv.offsetTop;
				px = px.add(-left,-top);
	    	}
...
function smActiveControl(ctls) {
	...
	for(var i in map.controls) {
		if(map.controls[i].type == SuperMap.Control.TYPE_TOOL && map.controls[i].active) {
